...
class MonitorTableModel(QAbstractTableModel):
    ...
    def __init__(self, rows: list[RuleRunRow] | None = None):
        super().__init__()
        self.rule_rows: list[RuleRunRow] = rows if rows is not None else []
        self.row_by_guid: dict[str, int] = {
            row.rule_guid: index for index, row in enumerate(self.rule_rows)
        }
    ...
    def upsert_row(self, rule_row: RuleRunRow) -> None:
        existing_index = self.row_by_guid.get(rule_row.rule_guid)
        ...
        self.row_by_guid[rule_row.rule_guid] = row_index
        self.endInsertRows()

    def add_row(self, rule_row: RuleRunRow):
    ...
    def data(self, index: QModelIndex, role=Qt.DisplayRole):
        ...
        if role == Qt.DisplayRole:
            if index.column() == 0:
                return rule_row.rule_guid
            elif index.column() == 1:
                return rule_row.rule_name
            elif index.column() == 2:
                return rule_row.status.value
            elif index.column() == 3:
                return rule_row.scope.value if rule_row.scope else ""
            elif index.column() == 4:
                return rule_row.task.value if rule_row.task else ""
            elif index.column() == 5:
                return rule_row.detail_type
            elif index.column() == 6:
                return rule_row.index
            elif index.column() == 7:
